...
import automedts.pipeline.create_searchspace_util
from automedts.askl_typing import FEAT_TYPE_TYPE

from .components.base import automedtsChoice, automedtsComponent
...
class BasePipeline(Pipeline):
    ...
    def set_hyperparameters(
        ...
        for node_idx, n_ in enumerate(self.steps):
            ...
            if isinstance(
                node, (automedtsChoice, automedtsComponent, BasePipeline)
            ):
    ...
    def get_hyperparameter_search_space(
        ...
        cs : ConfigSpace.configuration_space.Configuration
            The configuration space describing the automedtsClassifier.
    ...
    def _get_base_search_space(
        ...
        matches = automedts.pipeline.create_searchspace_util.get_match_array(
            pipeline=pipeline,
        ...
        for node_idx, n_ in enumerate(pipeline):
            ...
            is_choice = isinstance(node, automedtsChoice)
            ...
            #  choices are actually legal choices
                ...
                choices_list = (
                    automedts.pipeline.create_searchspace_util.find_active_choices(
                        matches,
                        node,
                        node_idx,
                        dataset_properties,
        ...
        if np.sum(matches) < np.size(matches):
            cs = automedts.pipeline.create_searchspace_util.add_forbidden(
                conf_space=cs,
                pipeline=pipeline,
                matches=matches,
                dataset_properties=dataset_properties,
    ...
    def _check_init_params_honored(self, init_params):
        ...
        for key, value in init_params.items():
            ...
            if isinstance(node, automedtsComponent):
                node_dict = vars(node)
            elif isinstance(node, automedtsChoice):
    ...
    def _validate_include_exclude_params(self):
        ...
        supported_steps = {
            ...
            if isinstance(step[1], automedtsChoice)
        }
