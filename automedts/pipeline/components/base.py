...
from automedts.pipeline.constants import SPARSE
...
class automedtsComponent(BaseEstimator):
    @staticmethod
    ...
    def __str__(self):
        ...
        return "automedts.pipeline %s" % name


class IterativeComponent(automedtsComponent):
...
class IterativeComponentWithSampleWeight(automedtsComponent):
    def fit(self, X, y, sample_weight=None):
...
class automedtsClassificationAlgorithm(automedtsComponent):
...
class automedtsPreprocessingAlgorithm(TransformerMixin, automedtsComponent):
...
class automedtsRegressionAlgorithm(automedtsComponent):
    """Provide an abstract interface for regression algorithms in
    auto-sklearn.

    Make a subclass of this and put it into the directory
    `automedts/pipeline/components/regression` to make it available."""
