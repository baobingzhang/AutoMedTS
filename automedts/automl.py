...
from automedts.data.xy_data_manager import XYDataManager
from automedts.ensemble_building import EnsembleBuilderManager
...
from automedts.ensembles.singlebest_ensemble import SingleBestFromRunhistory
from automedts.evaluation import ExecuteTaFuncWithQueue, get_cost_of_crash
from automedts.evaluation.abstract_evaluator import _fit_and_suppress_warnings
from automedts.evaluation.train_evaluator import TrainEvaluator, _fit_with_budget
from automedts.metrics import (
    Scorer,
    _validate_metrics,
...
from automedts.pipeline.components.classification import ClassifierChoice
from automedts.pipeline.components.data_preprocessing.categorical_encoding import (
    OHEChoice,
)
from automedts.pipeline.components.data_preprocessing.minority_coalescense import (
    CoalescenseChoice,
)
from automedts.pipeline.components.data_preprocessing.rescaling import RescalingChoice
from automedts.pipeline.components.feature_preprocessing import (
    FeaturePreprocessorChoice,
)
from automedts.pipeline.components.regression import RegressorChoice
from automedts.smbo import AutoMLSMBO
from automedts.util import RE_PATTERN, pipeline
from automedts.util.dask import Dask, LocalDask, UserDask
from automedts.util.data import (
...
from automedts.util.logging_ import (
...
from automedts.util.progress_bar import ProgressBar
from automedts.util.smac_wrap import SMACCallback, SmacRunCallback
...
class AutoML(BaseEstimator):
    ...
    def fit(
        ...
        #   `automedtsClassifier`, leaving `AutoML` to just produce raw
    ...
    def _log_fit_setup(self) -> None:
        ...
        requirements = pkg_resources.resource_string("automedts", "requirements.txt")
    ...
    def refit(
        ...
        y = convert_if_sparse(y)  # automedts does not handle sparse y for now
    ...
    def score(self, X, y):
        ...
        # We train automedts with a encoded version of y,
    ...
    def show_models(self) -> dict[int, Any]:
        ...
        * ``"classifier"`` or ``"regressor"`` - The automedts wrapped classifier or regressor.
        ...
        for (_, model_id, _), model in models.items():
            ...
            if is_cv:
                ...
                for cv_model in cv_voting_ensemble.estimators_:
                    ...
                    model_type, automedts_wrapped_model = cv_model.steps[-1]
                    estimator[
                        f"sklearn_{model_type}"
                    ] = automedts_wrapped_model.choice.estimator
            ...
            # For any other strategy
                ...
                # Adding sklearn model to the model dictionary
                model_type, automedts_wrapped_model = model.steps[-1]
                model_dict[
                    f"sklearn_{model_type}"
                ] = automedts_wrapped_model.choice.estimator
