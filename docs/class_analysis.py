...
def window_and_balance(X, y, window_size=10, step_size=1,
    ...
    summary_df = pd.DataFrame({
        "窗口化后数量": pd.Series(window_count),
        "重采样后数量": pd.Series(resampled_count)
    }).fillna(0).astype(int)
...
if __name__ == "__main__":
    ...
    for strat in strategies:
        ...
        ymax = max(df["窗口化后数量"].max(), df["重采样后数量"].max()) * 1.05

        df["窗口化后数量"].plot.bar(ax=axes[0], title=f"{strat} strategy - Windowed Count")
        axes[0].set_ylim(0, ymax)
        axes[0].set_xlabel("Category")
        axes[0].set_ylabel("Count")

        df["重采样后数量"].plot.bar(ax=axes[1], title=f"{strat} strategy - Resampled Count")
